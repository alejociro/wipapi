...
<div>
    <div class="center">
        <p>Planeación area integrada: {{$areaPlan->areaPlan->area->name}}</p>
    </div>

    <!-- Título -->
    <h1 class="title">{{$areaPlan->areaPlan->name}} semana: {{$areaPlan->areaPlan->week}}</h1>
    ...
    @foreach($areaPlan->areaPlan->planCompetences()->with('competence', 'competence.subject')->get() as $competence)
    ...
    @foreach($areaPlan->areaPlan->planTopics()->with('topic', 'topic.subject')->get() as $topic)
    ...
    @foreach($areaPlan->areaPlan->planIndicators()->with('indicator', 'indicator.subject')->get() as $indicator)
    ...
    @foreach($areaPlan->areaPlan->planCreativeAgenda()->with('activities')->get() as $agenda)
    ...
    @foreach($areaPlan->areaPlan->planActivities as $activity)
    ...
    @foreach(json_decode($areaPlan->areaPlan->planReferences) as $reference)
    ...
    @foreach(json_decode($areaPlan->areaPlan->planTasks) as $task)
